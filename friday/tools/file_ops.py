...
_WORK = Path.cwd(); _MAX_SIZE = 5*1024*1024
@dataclass
class FileResult: success: bool; output: str = ""; data: any = None
class FileOps:
    def __init__(self, workspace: str | Path | None = None): self._workspace = Path(workspace) if workspace else _WORK
    def _resolve(self, path): p = Path(path); return p.resolve() if p.is_absolute() else (self._workspace / p).resolve()
    ...
    async def tree(self, root=None, depth: int = 2) -> FileResult:
        base = self._resolve(root) if root else self._workspace
        try:
            lines = [(("  "*(len(p.relative_to(base).parts)-1)) + p.name + ("/" if p.is_dir() else "")) for p in sorted(base.rglob("*")) if len(p.relative_to(base).parts) <= min(depth,5)]
    ...
    async def info(self, path) -> FileResult:
        ...
        try:
            if not p.exists(): return FileResult(success=False, output=f"Not found: {p}")
            s = p.stat(); return FileResult(success=True, output=str({"path":str(p),"type":"dir" if p.is_dir() else "file","size":s.st_size}), data={})
        except Exception as e: return FileResult(success=False, output=str(e))
