...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import friday.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from friday.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from friday.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from friday.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from friday.tools.agent_tools import AgentSendTool
...
def _get_memory_backend(request: Request):
    ...
    cannot be built because the mandatory ``friday_rust`` extension is not
    ...
    if backend is None:
        from friday.tools.storage._stubs import MemoryBackendUnavailable
...
async def memory_config(request: Request):
    ...
    ``backend_type: sqlite``) when the native ``friday_rust`` extension is
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.friday/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from friday.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP friday_requests_total Total requests processed",
            "# TYPE friday_requests_total counter",
            f"friday_requests_total {stats.get('total_requests', 0)}",
            "# HELP friday_tokens_total Total tokens generated",
            "# TYPE friday_tokens_total counter",
            f"friday_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP friday_latency_avg_ms Average latency in milliseconds",
            "# TYPE friday_latency_avg_ms gauge",
            f"friday_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
def _record_ws_trace(
    ...
    *,
    ...
    if trace_store is None or not result:
        return
    from friday.traces.collector import record_response_trace
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from friday.server.auth_middleware import websocket_authorized
...
def include_all_routes(app) -> None:
    ...
    from friday.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from friday.server.agent_manager_routes import (  # noqa: PLC0415
