...
from friday.server.models import (
    ChatCompletionChunk,
...
def _ensure_identity_prompt(messages: list[Message], app_config) -> list[Message]:
    """Prepend FRIDAY's identity system prompt when the client omits one.
    ...
    try:
        ...
        logging.getLogger("friday.server").debug(
        ...
        try:
            ...
            if query_text:
                ...
                if len(enriched) > len(messages):
                    from friday.server.models import ChatMessage
...
def _record_completed_exchange(
    ...
    *,
    ...
    try:
        if bus is not None:
            from friday.memory import publish_completed_exchange
...
def _handle_agent(
    ...
    *,
    ...
    if audio_path:
        ...
        from friday.server.models import AudioMeta
...
async def _handle_stream_tools(
    ...
    *,
    ...
    """
    from friday.server.cloud_router import is_cloud_model
    ...
    async def generate():
        ...
        try:
            ...
            logging.getLogger("friday.server").error(
                "Tool stream error: %s",
                exc,
                exc_info=True,
            )
...
async def _handle_stream(
    ...
    *,
    ...
    from friday.server.cloud_router import (
        is_cloud_model,
        stream_cloud,
        stream_local,
    )
...
async def list_models(request: Request) -> ModelListResponse:
    ...
    """
    from friday.server.cloud_router import is_cloud_model, list_local_models
