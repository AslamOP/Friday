...
logger = logging.getLogger("friday.server.agent_manager")
...
def _ensure_registries_populated() -> None:
    ...
    from friday.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import friday.channels  # noqa: F401
    except Exception:
        pass
    ...
    # Also try to import browser tools (not included in friday.tools.__init__)
    for _browser_mod in ("friday.tools.browser", "friday.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("friday.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from friday.core.credentials import TOOL_CREDENTIALS
    from friday.core.registry import ChannelRegistry, ToolRegistry
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from friday.core.types import ToolCall as MsgToolCall
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from friday.channels.sendblue import (
                        SendBlueChannel,
                    )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from friday.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from friday.channels.slack_daemon import (
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            from friday.core.config import load_config
            from friday.core.paths import get_config_dir
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from friday.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from friday.core.credentials import get_credential_status
