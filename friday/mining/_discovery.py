# src/friday/mining/_discovery.py
...
from friday.mining._models import get_pearl_model_spec, pearl_variant_for_base_model
from friday.mining._stubs import MiningCapabilities
...
def detect_for_engine_model(
    *,
    ...
    if spec is None:
        return MiningCapabilities(
            False,
            reason=f"model {model!r} is not in FRIDAY' Pearl model registry",
        )
...
def check_disk_free(path: Path) -> Tuple[bool, str]:
    from friday.mining._constants import MIN_FREE_DISK_GB
