# src/friday/mining/vllm_pearl.py
...
from friday.mining._discovery import detect_for_engine_model
from friday.mining._docker import PearlDockerLauncher
from friday.mining._metrics import parse_gateway_metrics, parse_vllm_metrics
from friday.mining._stubs import (
    MiningCapabilities,
    MiningConfig,
    MiningProvider,
    MiningStats,
    ...
    async def start(self, config: MiningConfig) -> None:
        if isinstance(config.submit_target, PoolTarget):
            raise NotImplementedError(
                "pool support is v2 — see friday#XYZ. v1 only accepts "
                "submit_target='solo'."
            )
