...
@dataclass
class IndexReport:
    total_files: int = 0; indexed: int = 0; skipped: int = 0; errors: int = 0
    paths: list[str] = field(default_factory=list)
_IGNORE = {"__pycache__", ".git", ".venv", "node_modules", ".pytest_cache", "data"}
_TEXTS = {".py", ".js", ".ts", ".html", ".css", ".md", ".json", ".txt", ".sh"}
class FileIndexer:
    def __init__(self):
        self._kg = KnowledgeGraph(); self._vs = VectorStore(); self._seen = set()
    ...
    async def index_directory(self, path, recursive=True):
        ...
        for entry in sorted(root.glob("**/*" if recursive else "*")):
            ...
            if any(p.startswith(".") or p in _IGNORE for p in rel.parts): continue
            report.total_files += 1
            if entry.suffix not in _TEXTS: report.skipped += 1; continue
            if await self.index_file(entry): report.indexed += 1; report.paths.append(str(rel))
