...
both forced by the FRIDAY environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. FRIDAY
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — FRIDAY scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    FRIDAY substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip()
        + "\n\n"
