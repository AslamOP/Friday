...
Why not just use FRIDAY's :class:`InferenceEngine` for both? Two reasons:
...
from friday.agents.hybrid._openai_retry import patch_openai_globally
from friday.agents.hybrid._prices import (
    NO_TEMP_PREFIXES,
    is_gpt5_family,
...
def tavily_search_context(
    ...
    *,
    ...
    """Run FRIDAY WebSearchTool and return accounting-friendly metadata."""
...
class LocalCloudAgent(BaseAgent):
    ...
    def _call_openrouter(
        ...
        *,
        ...
        pass the FRIDAY-engine-style ``"openrouter/<provider>/<model>"``
        prefix (see ``src/friday/engine/cloud.py``) — we strip it here.
