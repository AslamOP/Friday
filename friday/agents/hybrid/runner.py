...
::
    ...
    python -m friday.agents.hybrid.runner --cell minions-gaia-qwen27b-opus-3
...
the registered agent, loads bench tasks via FRIDAY's existing dataset
...
existing rescore / dashboard scripts can read FRIDAY cells without
modification.
"""
...
from friday.agents.hybrid._energy import EnergyCollector
from friday.agents.hybrid._prompts import format_prompt as _format_prompt
...
def _load_gaia_tasks(n: Optional[int]) -> List[Dict[str, Any]]:
    ...
    for rec in ds.iter_records():
        ...
        # FRIDAY prefixes with `gaia-`) so subsets keyed by the upstream
        # id round-trip.
        md = rec.metadata or {}
...
def _get_gaia_scorer():
    ...
    if _GAIA_SCORER is None:
        with _GAIA_SCORER_LOCK:
            if _GAIA_SCORER is None:
                from friday.evals.backends.jarvis_direct import (
                    JarvisDirectBackend,
                )
...
def _score_gaia(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    Uses the shared FRIDAY :class:`GAIAScorer`. The previous version
...
def _score_swebench(
    ...
    """
    from friday.evals.core.types import EvalRecord
    from friday.evals.scorers.swebench_harness import (
...
def _build_agent(cell: Dict[str, Any]):
    ...
    import friday.agents  # noqa: F401 — populate registry
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m friday.agents.hybrid.runner",
