...
harness — each is registered as a standard FRIDAY agent so the rest
...
(defaults to ``~/.friday/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These FRIDAY ports
...
`src/friday/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
```bash
src/friday/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.friday/experiments/hybrid/docs/results.md`) at full N —
...
these FRIDAY cells should reproduce the harness numbers within
