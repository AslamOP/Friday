...
The wrapper adapts FRIDAY traces into ACE's
...
sidecar overlay under ``~/.friday/learning/ace/<task>/`` for the
...
from friday.core.config import ACEOptimizerConfig
...
class _TraceDataProcessor:
    """Adapter that exposes FRIDAY traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert FRIDAY trace records to ACE's sample dict shape."""
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_ACE:
            return {
                "status": "error",
                "reason": (
                    "ace not installed (pip install 'friday[learning-ace]')"
                ),
            }
