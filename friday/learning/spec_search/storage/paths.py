...
must NEVER be inside the FRIDAY source tree. ``resolve_spec_search_root``
...
from friday.core.paths import ConfigurationError, get_config_dir
from friday.security.file_utils import secure_mkdir

# ``ConfigurationError`` is re-exported from ``friday.core.paths`` (it used
...
def _resolve_friday_home() -> Path:
    """Resolve the FRIDAY home directory via the unified core resolver.
