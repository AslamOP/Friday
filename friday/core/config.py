...
if TYPE_CHECKING:
    ...
    # top level.
    from friday.mining._stubs import MiningConfig
...
# once at import via the env-aware resolver in ``friday.core.paths`` (the
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e friday[learning-ace]`` once the
    ...
    # most FRIDAY users have credentials for.
    ...
    # Empty string defaults to ``~/.friday/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "friday"
...
class StorageConfig:
    ...
    # the background and persists them across sessions (see friday.memory).
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        "You are FRIDAY, a helpful AI assistant running locally on the "
        ...
        "who or what you are, identify yourself as FRIDAY. Respond "
...
class AnalyticsConfig:
    ...
    usage events sent to the FRIDAY team's PostHog instance to
...
def _parse_mining_section(data: dict) -> Optional["MiningConfig"]:
    ...
    from friday.mining._stubs import MiningConfig, PoolTarget, SoloTarget
