...
class ContextEngine:
    def __init__(self):
        ...
        self._semantic = SemanticStore(); self._kg = KnowledgeGraph(); self._profile = UserProfile()
    async def load_context(self, user_input: str, intent: Intent) -> Context:
        ...
        if kg: mem["knowledge_graph"] = [{"id": e["id"], "type": e["type"]} for e in kg[:5]]
        sem = await self._semantic.search(user_input, top_k=3)
        if sem: mem["semantic"] = [{"id": r["id"], "text": r["text"][:200], "score": r["score"]} for r in sem]
