"""Central, env-aware resolution of FRIDAY' home directory.

FRIDAY keeps all of its runtime state (config, databases, caches, logs,
...
2. ``$XDG_DATA_HOME/friday`` — when ``$XDG_DATA_HOME`` is set, follow the
   XDG Base Directory spec by nesting a single ``friday`` directory under
...
3. ``~/.friday`` — the historical default. With no env vars set, the
...
override. Modules that previously hardcoded ``Path.home() / ".friday"``
should call :func:`get_config_dir` (or :func:`get_data_dir` /
:func:`get_cache_dir`) instead.

Defense in depth: the resolved root must never live inside the FRIDAY
...
_DEFAULT_DIR_NAME = ".friday"
_XDG_SUBDIR_NAME = "friday"
...
def _find_source_root() -> Path | None:
    ...
    Returns the directory containing the FRIDAY ``pyproject.toml`` (the one
    whose ``name = "friday"``), or ``None`` when running from an installed
...
def _reject_source_tree(path: Path) -> Path:
    """Raise if ``path`` resolves inside the FRIDAY source tree."""
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"FRIDAY home ({path}) is inside the source tree "
                f"({source_root}). FRIDAY refuses to write runtime state "
...
def get_config_dir() -> Path:
    """Resolve FRIDAY' single root directory, honoring env overrides.

    Precedence: ``$OPENJARVIS_HOME`` > ``$XDG_DATA_HOME/friday`` >
    ``~/.friday``. The result is always absolute and is rejected if it
    falls inside the FRIDAY source tree.
    """
...
def get_config_path() -> Path:
    """Resolve the path to ``config.toml`` under the FRIDAY root."""
...
def get_cache_dir() -> Path:
    ...
    Lives at ``<root>/cache`` so caches stay inside the single FRIDAY
