...
class KnowledgeGraph:
    ...
    def add_relation(self, src: str, tgt: str, rel: str):
        self._relations.append({"source": src, "target": tgt, "relation": rel})
    def query_entity(self, eid: str): return self._graph.get(eid)
    def search(self, query: str) -> list[dict]:
        q = query.lower()
        results = []
        for e in self._graph.values():
            if q in e["id"].lower() or q in e["type"].lower():
                results.append(e)
                continue
