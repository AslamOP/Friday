...
class EntityExtractor:
    ...
    async def extract_from_conversation(self, conv: list[dict]) -> int:
        total = 0
        for msg in conv[-20:]:
            total += await self.extract_and_store(msg.get("content", ""), msg.get("role", "unknown"))
        return total

    def get_stats(self) -> dict:
        return {"known_entities": len(self._known)}
