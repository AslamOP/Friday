"""Detect how FRIDAY was installed so we can show the right upgrade
command (and run the right upgrade command for ``jarvis self-update``).

Three install paths are supported today:

- **PyPI** (``pip install friday``). The package lives somewhere
  inside ``site-packages``. Upgrade with ``pip install --upgrade friday``.
- **uv tool** (``uv tool install friday``). Lives in a uv-managed
  isolated venv under ``~/.local/share/uv/tools/``. Upgrade with
  ``uv tool upgrade friday``.
...
We detect by inspecting ``friday.__file__``. If we can't tell with
...
def detect_install() -> InstallInfo:
    ...
    Cheap: just walks the parent directories of ``friday.__file__``
    once and checks for marker directories. No subprocess calls.
    """
    try:
        import friday

        pkg_file = Path(friday.__file__).resolve()
    except Exception:
        return InstallInfo(
            kind="unknown",
            upgrade_command="pip install --upgrade friday",
        )
