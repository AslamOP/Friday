...
@config.command("path")
def show_path() -> None:
    """Print the resolved FRIDAY directories (home, config, cache).

    All FRIDAY state lives under a single root, resolved in priority
    order: ``$OPENJARVIS_HOME`` > ``$XDG_DATA_HOME/friday`` >
    ``~/.friday``. Use this to confirm where your data is stored after
    setting an override.
    """
    from friday.core.paths import get_cache_dir, get_config_dir, get_config_path
