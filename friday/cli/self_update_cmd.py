"""`jarvis self-update` — upgrade FRIDAY to the latest release.

Runs the right upgrade command for how the user installed FRIDAY:

- PyPI installs get ``pip install --upgrade friday``.
- uv-tool installs get ``uv tool upgrade friday``.
...
import friday
from friday.cli._install_detect import detect_install


@click.command(
    "self-update",
    help=(
        "Upgrade FRIDAY to the latest release. Detects how you "
...
def self_update(check: bool, yes: bool) -> None:
    ...
    click.echo(f"Current FRIDAY version: v{current}")
